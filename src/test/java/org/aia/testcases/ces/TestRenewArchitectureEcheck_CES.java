...
import org.aia.pages.api.ces.FontevaProviderAppStatus;
...
public class TestRenewArchitectureEcheck_CES extends BaseClass {
	...
	SignUpAPI signUpAPI;
	FontevaProviderAppStatus providerAppStatus;
	...
	public void setUp() throws Exception {
		...
		providerAppStatus = PageFactory.initElements(driver, FontevaProviderAppStatus.class);
	}

	@Test(priority = 1, description = "Renew Online Architecture Firm (E-check).", enabled = true, groups= {"Smoke"})
	public void ValidateRenewECheckArchitecture() throws Throwable {
		String prefix = "Dr.";
		String suffix = "Sr.";
		...
		// Get Provider application IDution
...
//		driver.get("https://aia--upgradestg.sandbox.my.salesforce.com/secur/frontdoor.jsp?sid=" + sID);
		providerAppStatus.changeProviderApplicationStatus(dataList.get(3),"Approved");
//		fontevaPage.changeProviderApplicationStatus(userAccount.get(0)+" "+userAccount.get(1), paId, "Approved");
		...
		checkOutPageCes.validatePaymentProcessDone();
...
//		 fontevaPage.changeTermDates(dataList.get(0)+" "+dataList.get(1));
		termDateChangeAPICall.changeTermDateAPI(dataList.get(3), "2024-12-31","CES Architecture Firm");
		//Make sure with the renew date in above line
		// Navigate back to renew CES portal
		driver.get("https://aia--upgradestg.sandbox.my.site.com/Providers/s/renew");
//		driver.switchTo().alert().accept();
		...
		Thread.sleep(40000);
		String bankPayAmount = util.replaceStringvalues(util.getPDFReceiptValue("Total"));
		System.out.println("Original receipt" + util.getPDFReceiptValue("Receipt Number"));
		String bankPayReciptData = util.replaceStringvalues(util.getPDFReceiptValue("Receipt Number")); 
		util.deleteFile();
		Reporter.log("LOG : INFO -Receipt Number is" + bankPayReciptData);
		Reporter.log("LOG : INFO -Customer AIA Number is : " + userAccount.get(1));
		...
		apiValidation.verifyReciptDetails(bankPayReciptData, amount, "CES Architecture Firm");
