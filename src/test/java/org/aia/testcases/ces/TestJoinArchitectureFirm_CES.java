...
public class TestJoinArchitectureFirm_CES extends BaseClass {
	...
	@Test(priority = 1, description = "FC-112, 120:-Validate Online JOIN for Architecture Firm using credit card.", enabled = true, groups = {
			"Smoke" })
	public void ValidateJoinArchitectureFirm() throws Throwable {
		String prefix = "Dr.";
		String suffix = "Sr.";
...
//		driver.get("https://aia--upgradestg.sandbox.my.salesforce.com/secur/frontdoor.jsp?sid=" + sID);
		// driver.get(DataProviderFactory.getConfig().getValue("fonteva_endpoint_upgradestg"));
//		fontevaPage.changeProviderApplicationStatus(userAccount.get(0) + " " + userAccount.get(1), paId, "Approved");
		...
		// validations
		apiValidation.verifyProviderApplicationDetails("Approved", dataList, "Architecture Firm",
				userAccount.get(0) + " " + userAccount.get(1), true, java.time.LocalDate.now().toString(),
				"AutomationOrg", "Architecture Firm", "No");

		// Validate CES Provider account details - Fonteva API validations
		apiValidation.verifyProviderApplicationAccountDetails("Active", "CES Architecture Firm", "2025-12-31", false);
