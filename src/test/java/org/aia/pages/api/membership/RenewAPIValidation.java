...
public class RenewAPIValidation {
	...
	public void verifyMemebershipRenewal(String memberAccount, String enddate, String type,
		...
		while ((totalMembershipCount == 0) && retryCount < 10) {
			...
			if ((totalMembershipCount == 0) && accountID != null) {
				String SUBSCRIPTIONS_URI = "https://aia--upgradestg.sandbox.my.salesforce.com/services/data/v56.0/sobjects/Account/"
						+ accountID + "/OrderApi__Subscriptions__r";
				...
				response = given().header("Authorization", "Bearer " + bearerToken)
						...
						.header("Accept-Encoding", "gzip, deflate, br").header("Accept-Encoding", "keep-alive")
				...
				jsonPathEval = response.jsonPath();
				System.out.println("My original resp:" + jsonPathEval.prettyPrint());
				totalMembershipCount = jsonPathEval.getInt("totalSize");
				// Thread.sleep(50000);
			}
			retryCount = retryCount + 1;
			System.out.println("Didn't get response after:" + retryCount);
			...
			if (totalMembershipCount > 0
					...
					&& util.containsExpectedValue(response, "records[0].OrderApi__Term_End_Date__c", enddate)) {
		...
		System.out.println("I retried For:" + retryCount);
	...
	public void verifySalesOrder(String orderPaidStatus, String closed, Object dues, String posted)
		...
		String query = "SELECT Id,Name,OrderApi__Sales_Order_Status__c, AIA_Origin__c, OrderApi__Status__c, OrderApi__Posting_Status__c, OrderApi__Amount_Paid__c, OrderApi__Date__c, AIA_National_Subscription_Plan__c,LastModifiedDate FROM OrderApi__Sales_Order__c WHERE OrderApi__Account__c='"
				+ accountID + "' ORDER BY LastModifiedDate DESC";
		...
		jsonPathEval = response.jsonPath();
		System.out.println("Sales order response" + jsonPathEval.prettyPrint());
		int totalSalesOrderCount = jsonPathEval.getInt("totalSize");
		String Origin_renew = jsonPathEval.getString("records[0].AIA_Origin__c");
		String Origin = jsonPathEval.getString("records[1].AIA_Origin__c");
		...
		if (totalSalesOrderCount > 0) {
			...
			} else if (Origin_renew.contains("Renewal")) {
...
//			}
	...
	public void verifyReciptDetails(Object receipt, Object feePaid) throws InterruptedException {
	    // SOQL Query URL with dynamic Account ID
	    String RECIPTS_URI = "https://aia--upgradestg.sandbox.my.salesforce.com/services/data/v56.0/query";
	    // Endpoint and Query parameters
        String query = "SELECT Id,Name,OrderApi__Total__c,LastModifiedDate FROM OrderApi__Receipt__c WHERE OrderApi__Account__c='"+accountID+"' ORDER BY LastModifiedDate DESC";
	    
	    // Send GET request to Salesforce API
	    ...
	    System.out.println("Response: " + jsonPathEval.prettyPrint());
	    
	    // Get the total number of receipts for the account
	    int totalReciptCount = jsonPathEval.getInt("totalSize");

	    // Check if there are any receipts found
	    if (totalReciptCount > 1) {
	        System.out.println("Number of Recipts: " + totalReciptCount);

	        // Get the total fee paid for the most recent receipt
	        Double totalFeePaid = jsonPathEval.getDouble("records[0].OrderApi__Total__c");
	        String receiptNumber = jsonPathEval.getString("records[0].Name");

	        // Compare the fee paid with the expected fee
//	        if (totalFeePaid!=feePaid) {
//	            // If fees don't match, check the second most recent receipt
...
//	        }
	        ...
	        // Assertions to verify the receipt and fee paid
	 ...
	 * @throws InterruptedException
	 */
	public String validatePDFreceiptUsBankPayments() throws InterruptedException {
		// Use Account ID to fetch account details.
		String RECIPTS_URI = ACCOUNT_URI + "/" + accountID + "/OrderApi__Receipts__r";
		String pdfEncriptedId = null;
		Response response = given().header("Authorization", "Bearer " + bearerToken)
				.header("Content-Type", ContentType.JSON).header("Accept", ContentType.JSON)
				.param("fields", "Name," + "OrderApi__Total__c," + "OrderApi__Encrypted_Id__c").when().get(RECIPTS_URI)
				.then().statusCode(200).extract().response();
