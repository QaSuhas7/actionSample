...
public class ReJoinAPIValidation {
	...
	public void validateReJoinMemebership(String memberAccount, String enddate, String type,
		...
		while ((totalMembershipCount == 0 && retryCount < 10)) {
			...
			if (accountID != null) {
				...
				jsonPathEval = response.jsonPath();
				System.out.println("My resp:" + jsonPathEval.prettyPrint());
			...
			if (totalMembershipCount > 0
					&& util.containsExpectedValue(response, "records[1].OrderApi__Status__c", "Active")) {
...
//				assertEquals(termStartDate, activatedDate); //Uncomment after renewal period
	...
	public void verifySalesOrder(String orderPaidStatus, String closed, Object dues, String posted)
		...
		String query = "SELECT Id,Name,OrderApi__Sales_Order_Status__c, OrderApi__Status__c, OrderApi__Posting_Status__c, OrderApi__Amount_Paid__c, OrderApi__Date__c, AIA_National_Subscription_Plan__c,LastModifiedDate FROM OrderApi__Sales_Order__c WHERE OrderApi__Account__c='"
...
//We are removing Dues-In installment origin from this function, DIP is removed from functionality  
//			originRejoin = jsonPathEval.getString("records[0].AIA_National_Subscription_Plan__c");
//			origin = jsonPathEval.getString("records[1].AIA_National_Subscription_Plan__c");
...
//				postingStatus = jsonPathEval.getString("records[3].OrderApi__Posting_Status__c");
