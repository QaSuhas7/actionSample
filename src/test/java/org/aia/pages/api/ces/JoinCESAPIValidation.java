...
public class JoinCESAPIValidation {
		...
		while (retryCount < 10 && providerStat == false) {
			...
			if (providerID != null) {
				...
				response = given().header("Authorization", "Bearer " + bearerToken)
						...
						.get(providerUri).then().statusCode(200).extract().response();
				jsonPathEval = response.jsonPath();
	...
	public String getProviderApplicationID(String owner) throws InterruptedException {
		...
		String PA_CREATEDBY_URI = "https://aia--upgradestg.sandbox.my.salesforce.com/services/data/v56.0/query";
		...
		while (retryCount < 10 && providerStat == false) {
			...
			System.out.println(jsonPathEval.toString());
			providerID = jsonPathEval.getString("records[0].Id");
			System.out.println(providerID);
			providerName = jsonPathEval.getString("records[0].Name");
		...
		while (retryCount < 10 && providerStat == false) {
			...
			if (providerID != null) {
				...
				Response response2 = given().header("Authorization", "Bearer " + bearerToken)
						.header("Content-Type", ContentType.JSON).header("Accept", ContentType.JSON)
						.param("fields", "Name, " + "Application_Stage__c, " + "Application_Type__c, "
								+ "Primary_Contact_Name__c, " + "Electronic_Attestation__c, " + "Attestation_Date__c, "
								+ "Organization_Name__c, " + "	Organization_Type__c, " + "Prior_Provider__c, "
								+ "Former_CES_Provider_Number__c, " + "Website__c, " + "Street__c, "
								+ "ZIP_Postal_Code__c, " + "City__c, " + "Country__c, " + "OwnerId, "
								+ "LastModifiedById, " + "Telephone__c, " + "CreatedById")
	...
	public void verifyProviderApplicationAccountDetails(String cesproviderStatus, String cesmembershipType,
		...
		while (retryCount < 10 && providerStat == false) {
			...
			if (providerID != null) {
				...
				Response response2 = given().header("Authorization", "Bearer " + bearerToken)
						.header("Content-Type", ContentType.JSON).header("Accept", ContentType.JSON)
						.param("fields", "Account__c").when().get(PROVAPP_ID_URI).then().statusCode(200).extract()
						.response();
				...
				Response responseMembership = given().header("Authorization", "Bearer " + bearerToken)
						.header("Content-Type", ContentType.JSON).header("Accept", ContentType.JSON)
						.param("fields",
								"Id, " + "Name, " + "AIA_Ecommerce_Renew_Link__c, " + "AIA_CES_Renew_Eligible__c, "
										+ "AIA_CES_Subscription_Type__c, " + "Membership_Type__c, "
										+ "OrderApi__Term_End_Date__c, " + "OrderApi__Paid_Through_Date__c, "
										+ "OrderApi__Term_Start_Date__c, " + "OrderApi__Grace_Period_End_Date__c, "
										+ "OrderApi__Days_To_Lapse__c,OrderApi__Activated_Date__c")
				...
				String activationDate= jsonPathEval.getString("OrderApi__Activated_Date__c");
				...
				System.out.println("Membership Activation date: "+activationDate);
				System.out.println("=====================================");
...
//		assertEquals(termStartDate, java.time.LocalDate.now().toString());
				...
				String inputString1 = activationDate;
