...
public class PrimaryInformationPage {
	...
	String selectedCountry = "//div[text()='%s']";
...
//		String countryName = e1.getText();
		...
		List<WebElement> e2 = driver
				.findElements(By.xpath("(//div[@id='menu-country']//following-sibling::div)[last()-1]/ul/li"));

		for (WebElement webElement : e2) {
			util.waitUntilElement(driver, webElement);
			if (webElement.getText().equalsIgnoreCase("United States")) {
				util.waitUntilElement(driver, webElement);
				util.clickUsingJS(driver, webElement);
			}

			if (util.getCustomizedWebElement(driver, selectedCountry, "United States").isDisplayed())
				break;
	...
	public void enterAddressDetailsSanFrance() throws InterruptedException {
		...
		util.enterText(driver, country2, addressCountry);
		util.enterText(driver, zipCode2, addressZipCodepac);
	}
	...
	public void enterAdressDetailforDonation() {
		...
		util.enterText(driver, zipCode2, data.testDataProvider().getProperty("addressZipCode"));
	}
	...
	public void enterPrimaryInfoForNewChapterLogic(String radbtnString, String careerType) throws InterruptedException {
		selfDescribe_selectRadioBtnByValue(radbtnString);
		if (!radbtnString.contentEquals("allied")) {
			selectCountry();
			util.enterText(driver, zipCode1, newZipCode);
		}
