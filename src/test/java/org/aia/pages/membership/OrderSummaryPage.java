...
public class OrderSummaryPage {
	...
	@FindAll({@FindBy(xpath = "//ul[@class='checkout-summary']//li")}) 
	List<WebElement> paymentSubTotalList;
	...
	public Map<String, Double> paymentInfo(String installment) throws InterruptedException {
		Thread.sleep(30000);
		Map<String, Double>  paymentDetails = new HashMap<>();
		String[] parts = null;		
		for (int i = 0; i < paymentSubTotalList.size(); i++) {
			Thread.sleep(10000);
			String amount = paymentSubTotalList.get(i).getText();
			...
			parts = amount.split("\n");
	            ...
	            if (parts.length == 2) {
	                ...
	                System.out.println("Value: " + Double.parseDouble(value)); // Parsing to double for decimal values
	                paymentDetails.put(key, Double.parseDouble(value)); // Putting key-value pair in the map
	            } else {
	                System.out.println("Invalid format: Expected two parts separated by newline character.");
	            }
	        }
	        ...
	        // Accessing values from paymentDetails map
	        System.out.println("AIA National: $" + paymentDetails.get("AIA National"));
	        System.out.println("AIA California: $" + paymentDetails.get("AIA California"));
	        
	        // Adding values of AIA National and AIA California
	        double total;
	        if(installment == "full") {
	        total = paymentDetails.get("AIA National") + paymentDetails.get("AIA California") + paymentDetails.get("Supplemental Dues - San Francisco")+paymentDetails.get("Special Assessment for the Center for Architecture + Design")+paymentDetails.get("AIA San Francisco");
	        }else {
	        	total = paymentDetails.get("AIA National") + paymentDetails.get("AIA California") + paymentDetails.get("Supplemental Dues - San Francisco")+paymentDetails.get("Special Assessment for the Center for Architecture + Design")+paymentDetails.get("AIA San Francisco")+paymentDetails.get("Installment plan administration fee");
	        }
	        
	        System.out.println("Total: $" + total);

	        // Update paymentDetails with the total
	        paymentDetails.put("Total", total);
	        
	        // Printing updated paymentDetails
	        System.out.println("Updated paymentDetails: " + paymentDetails);
